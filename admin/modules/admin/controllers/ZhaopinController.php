...
class ZhaopinController extends BaseController{
    ...
    public function actionSave($id = 0){
        if(empty($id)){
            $model = new SysZhaopin();
        }else{
            $model = SysZhaopin::getModel($id);
        }
        ...
        if(Yii::$app->request->isPost){
            if($model->load(Yii::$app->request->post())){
                if($model->save()){
                    ...
                    return $this->redirect('/admin/zhaopin/save?id='.$model->id);
                }else{
    ...
    public function actionDel($id = null){
        ...
        Yii::$app->db->createCommand()->delete(SysZhaopin::tableName(),'id=:id',[':id'=>$id])->execute();
